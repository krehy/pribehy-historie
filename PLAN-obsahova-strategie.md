...
## 9i. Interaktivní příběh vs. článek + autorský toolpanel (rozhodnuto 2026-07-02)

- **Rozlišení obsahu:** `interaktivní` (má `beats`/`hero` — kinematický zážitek) vs `článek`
  (jen `body` text). Odvodit z dat (`isInteractive = !!story.beats`); web odliší odznakem.
  Krátká úroveň = článek; vlajkové = interaktivní (viz 9b dvouúrovňový obsah).
- **Autorský toolpanel:** plovoucí panel ve stylu dev Tweaks (tmavě-žlutý), **jen role `author`**,
  **kontextový podle stránky** — samostatné ikonky/akce (autorův „tool belt").
- **Kontextová akce na článku:** je-li otevřený článek a **neexistuje k němu interaktivní verze**,
  toolpanel nabídne **„Vytvořit interaktivní příběh"** → odkaz **do creatoru s kontextem článku**
  (`/studio/editor/new?from=<slug>`), aby autor rovnou navázal.
